...
- **Gender slots:** `female_slots` / `male_slots` **are enforced in team selection as of TC1.4b** — `allocateInstitutionPrizes` passes the group's configured values into `computeTeamScoresWithReasons` as `slots`. Under **RULING 1** a female slot is satisfied only by an explicit `'F'`, and a male slot is satisfied by anyone **not** recorded as `'F'` — including players whose gender was never recorded, because a blank Sex column in a Swiss-Manager export is the default for a male entrant, not missing data. Both are **minimums, not exact quotas**: once each minimum is met, every remaining board goes to the best-ranked players still unselected, of any gender. An institution that cannot meet a minimum is excluded with `female_slots_unfilled` or `male_slots_unfilled`. **Display has not caught up.** The editor badge (`TeamPrizesEditor.tsx:243-247`), the results panel (`TeamPrizeResultsPanel.tsx:86-92`) and the PDF (`generatePdf/index.ts:408-413`) still print `2F + 2M required`, which **RULING 2 forbids** and which misdescribes a minimum as a quota. TC1.5 replaces those four text-producing files with one formatter. Do not read enforcement being live as meaning the printed claim is correct yet. Rulings are recorded in `docs/team-championship/PRD.md`.

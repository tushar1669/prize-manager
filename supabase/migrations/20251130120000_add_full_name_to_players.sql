-- Add full_name column to players for displaying unabridged names
ALTER TABLE public.players
ADD COLUMN IF NOT EXISTS full_name text;

COMMENT ON COLUMN public.players.full_name IS 'Organizer-provided full name (falls back to name when absent)';

-- Backfill existing rows to avoid null display regressions
UPDATE public.players
SET full_name = name
WHERE full_name IS NULL;

-- Update import_replace_players to persist full_name alongside name

-- Corrective forward-only re-assertion of canonical tournament access resolver.
-- This is intentionally idempotent-safe against prior drifted definitions in production.
DROP FUNCTION IF EXISTS public.get_tournament_access_state(uuid);

CREATE FUNCTION public.get_tournament_access_state(tournament_id uuid)
RETURNS TABLE (
...
BEGIN
  ...
  is_free_small_tournament := (v_players_count BETWEEN 1 AND v_free_player_threshold);
